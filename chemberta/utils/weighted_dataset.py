...
class WeightedRawTextDataset(Dataset):
    """
    ...
    def __init__(self, tokenizer, file_path: str, block_size: int):
        ...
        self.dataset = datasets.load_dataset("csv", data_files=data_files)["train"]
        if 'p' in self.dataset.features:
            self.p = torch.tensor(self.dataset['p'])
    ...
    def preprocess(self, feature_dict):
        ...
        return {'input_ids': torch.tensor(batch_encoding["input_ids"])}
...
class RegressionTextIterable(torch.utils.data.IterableDataset):
    def __init__(self, tokenizer, file_path: str, block_size: int):
        ...
        self.dataset = datasets.load_dataset("text", data_files=get_data_files(file_path))[
            "train"
        ]
...
class LazyRegressionDataset(Dataset):
    ...
    def __init__(self, tokenizer, file_path: str, block_size: int):
        ...
        data_files = get_data_files(file_path)
        self.dataset = datasets.load_dataset("text", data_files=data_files)["train"]
